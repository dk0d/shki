...
pub fn generate_rust_code(snapshot: &Snapshot, config: &CodegenConfig) -> Result<GeneratedCode> {
    let mut code = GeneratedCode::new();

    // Generate enums first (structs may depend on them)
    for (name, enum_snapshot) in &snapshot.enums {
        let rust_enum = generate_enum(name, enum_snapshot, config);
        code.enums.insert(name.clone(), rust_enum);
    }

    // Generate structs from tables
    for (name, table_snapshot) in &snapshot.tables {
        let rust_struct = generate_struct(name, table_snapshot, &snapshot.enums, config);
...
fn generate_struct(
    ...
    let fields: Vec<RustField> = table
        .columns
        .values()
        .map(|col| generate_field(col, enums, config))
        .collect();
...
fn generate_field(
    ...
    let rust_type = sql_type_to_rust(&col.data_type, col.nullable, enums, config);
...
fn sql_type_to_rust(
    ...
    if let Some(inner) = sql_type.strip_suffix("[]") {
        let inner_rust = sql_type_to_rust(inner, false, enums, config);
...
fn make_safe_field_name(name: &str) -> String {
...
impl RustEnum {
    /// Generate the Rust code for this enum
...
impl RustStruct {
    /// Generate the Rust code for this struct
    pub fn to_tokens(&self) -> TokenStream {
        let name = Ident::new(&self.name, Span::call_site());
...
mod tests {
    ...
    fn test_sql_type_to_rust() {
        ...
        assert_eq!(sql_type_to_rust("INTEGER", false, &enums, &config), "i32");
        assert_eq!(
            sql_type_to_rust("TEXT", true, &enums, &config),
            "Option<String>"
        );
        assert_eq!(
            sql_type_to_rust("TIMESTAMP WITH TIME ZONE", false, &enums, &config),
            "chrono::DateTime<chrono::Utc>"
        );
        assert_eq!(
            sql_type_to_rust("UUID", false, &enums, &config),
            "uuid::Uuid"
        );
    }
}
